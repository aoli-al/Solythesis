...
bench = Bench(args.endpoint, args.path, 'CryptantCrabNFT', args.pow)
...
NUM_OF_CONTRACT = 132
dd_addr = [bench.call_contract_function(contract_creator[0], 'constructor', ["Crab", "CC"],
...
for i in range(ITER):
    ...
    for k in range(NUM_OF_CONTRACT):
        result = bench.call_contract_function(contract_creator[0], 'mintToken',
                                     [users[0], i],
        ...
        if gas >= 8000000:
            ...
            gas = 0
    for k in range(NUM_OF_CONTRACT):
        result = bench.call_contract_function(users[0], 'transferFrom', [users[0], users[1], i], dd_addr[k])
        gas += TRANSFER
        if gas >= 8000000:
            bench.wait_for_result(result, check_successful=True)
            num_of_blocks += 1
