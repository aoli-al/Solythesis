...
bench = Bench(args.endpoint, args.path, 'MutantMonsters', args.pow)
...
NUM_OF_CONTRACT = 132
dd_addr = [bench.call_contract_function(contract_creator[0], 'constructor', [],
...
for i in range(ITER):
    ...
    for k in range(NUM_OF_CONTRACT):
        result = bench.call_contract_function(contract_creator[0], 'mintTokenForClient',
                                     [users[0], i],
