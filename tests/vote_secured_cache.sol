...
contract AdvancedTokenVote1202 {
  ...
  function createIssue (uint issueId, address _tokenAddr, uint[] memory options_, address[] memory qualifiedVoters_, string memory issueDesc_) public {
    ...
    for (uint256 index_10 = 0; index_10 < x_arr_0.length; index_10 += 1) {
      assert(sum_votes[x_arr_0[index_10]][y_arr_1[index_10]] == weightedVoteCounts[x_arr_0[index_10]][y_arr_1[index_10]]);
    }
  ...
  function vote (uint issueId, uint option) public returns (bool success) {
    ...
    uint256 weight = weights[issueId][msg.sender];
    {
      ...
      {
        x_arr_0.push(issueId);
        y_arr_1.push(ballots[issueId][msg.sender]);
      }
    ...
    {
      for (uint256 index_21 = 0; index_21 < x_arr_0.length; index_21 += 1) {
        assert(sum_votes[x_arr_0[index_21]][y_arr_1[index_21]] == weightedVoteCounts[x_arr_0[index_21]][y_arr_1[index_21]]);
      }

      return true;
    }

    for (uint256 index_22 = 0; index_22 < x_arr_0.length; index_22 += 1) {
      assert(sum_votes[x_arr_0[index_22]][y_arr_1[index_22]] == weightedVoteCounts[x_arr_0[index_22]][y_arr_1[index_22]]);
    }
